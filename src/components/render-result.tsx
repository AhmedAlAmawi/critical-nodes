...
import { Download, Copy, X, RotateCcw, AlertCircle } from "lucide-react";
...
export function RenderResult({ mode = "inline", onClose }: RenderResultProps) {
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center max-w-xs">
          ...
          <p className="text-[12px] text-muted-foreground/30 leading-relaxed">
            Complete the prompt node and generate to see your render on the canvas.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="w-full max-w-xs space-y-4">
            <div className="flex justify-between text-[11px] text-muted-foreground">
              ...
              <span className="font-mono tabular-nums">{state.renderProgress}%</span>
            </div>
            <Progress value={state.renderProgress} className="h-[3px]" />
            <p className="text-[11px] text-muted-foreground/30 text-center">This may take 30-60 seconds</p>
          </div>
        </div>
      )}
      ...
      {state.renderError && (
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="max-w-sm text-center space-y-3">
            ...
            <p className="text-[13px] text-destructive/80">{state.renderError}</p>
            <Button variant="ghost" size="sm" className="text-[11px]" onClick={handleClear}>
      ...
      {dataUrl && (
        ...
        >
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={dataUrl} alt="Rendered result" className="max-w-full max-h-full object-contain rounded-md" />
        </motion.div>
      )}
    </div>
  );
